...
from datasets._3dfront import _3DFront
...
def calibrate_neighbors(dataset, config, collate_fn, keep_ratio=0.8, samples_threshold=2000):
    ...
    for i in range(len(dataset)):
        # src_pcd, tgt_pcd, src_feats, tgt_feats, correspondences, rot, trans, gt_cov = dataset[i]
        # print(src_pcd.shape, tgt_pcd.shape, rot, trans)
...
def get_datasets(config):
    ...
    elif(config.dataset == '3dfront'):
        train_set = _3DFront(config, 'train', data_augmentation=True)
        val_set = _3DFront(config, 'val', data_augmentation=False)
        test_set = _3DFront(config, 'test', data_augmentation=False)  
    else:
...
def get_dataloader(dataset, config, shuffle=False, neighborhood_limits=None, sampler=None):
